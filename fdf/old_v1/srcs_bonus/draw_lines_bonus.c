...
static double	ft_find_max(double x, double y)
{
	if (x < 0)
		x = -x;
	if (y < 0)
...
{
	data->x = (data->x - data->y) * sin(data->angle_x) - (z * data->z_cor);
	data->y = (data->x + data->y) * cos(data->angle_y) - z;
	data->x1 = (data->x1 - data->y1) * sin(data->angle_x) - (z1 * data->z_cor);
